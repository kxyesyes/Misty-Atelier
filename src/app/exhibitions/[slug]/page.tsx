  ...
  return (
    <main className="ink-paper-fibers min-h-screen overflow-hidden text-ink">
      <div className="page-x mx-auto max-w-7xl py-12 sm:py-16">
        <FadeIn delay={0.1}>
          <nav className="mb-16">
            <Link href="/" className="inline-flex items-center gap-2 text-rain hover:text-cinnabar transition-colors text-sm tracking-widest uppercase">
          ...
          <header
            className="ink-panel mb-16 grid gap-10 overflow-hidden border-y border-ink/10 py-8 md:grid-cols-[1fr_0.62fr] md:items-center md:py-12"
            style={{ color: ex.palette.foreground }}
          >
            <div className="flex-1 px-1">
              ...
              <div className="mt-8 flex flex-wrap gap-2">
                {ex.keywords.map((keyword) => (
                  <span
                    key={keyword}
                    className="border px-2.5 py-1.5 font-sans text-xs leading-none tracking-wide"
                    style={{ borderColor: "color-mix(in srgb, var(--color-cinnabar) 55%, transparent)", color: ex.palette.foreground }}
                  >
                    {keyword}
                  </span>
                ))}
              ...
              <div className="mt-8 flex gap-4">
                {Object.entries(ex.palette).map(([name, color]) => (
                  <div
                    ...
                    className="h-8 w-8 border border-ink/10 shadow-inner"
        ...
        {nextEx && (
          <FadeIn delay={0.3} className="mt-32 pt-16 border-t border-mist flex flex-col items-center">
            <span className="text-xs uppercase tracking-widest text-rain mb-4">Next Exhibition</span>
            <Link href={`/exhibitions/${nextEx.slug}`} className="text-2xl font-serif hover:text-cinnabar transition-colors">
              {nextEx.titleEn} &rarr;
            </Link>
          </FadeIn>
        )}
