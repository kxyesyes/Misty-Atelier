...
export function WeatherDial() {
  ...
  return (
    <div className="ink-panel fixed bottom-20 right-4 z-50 flex flex-col gap-1.5 p-1.5 sm:right-6">
      {OPTIONS.map((option) => {
        ...
        return (
          <button
            ...
            className={`flex h-9 w-9 items-center justify-center border border-transparent transition-colors ${
              active
                ? "border-cinnabar bg-cinnabar text-paper"
                : "text-rain hover:border-cinnabar hover:bg-mist/60 hover:text-ink"
            }`}
          >
