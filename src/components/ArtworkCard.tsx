...
export function ArtworkCard({
  ...
  return (
    ...
    >
      <div
        className={`floating-art parallax-art relative mb-4 overflow-hidden bg-cream transition duration-700 group-hover:-translate-y-1 ${frameClass}`}
      >
        <Image
          ...
          className={`${imageFitClass} grayscale-[10%] sepia-[8%] transition duration-1000 group-hover:scale-[1.025] group-hover:grayscale-0`}
          quality={variant === "feature" ? 92 : 86}
        />
        {!compact && (
          <span className="ink-panel absolute bottom-3 left-3 max-w-[calc(100%-1.5rem)] translate-y-2 px-3 py-2 font-sans text-[10px] uppercase tracking-[0.18em] text-ink opacity-0 transition duration-500 group-hover:translate-y-0 group-hover:opacity-100 group-focus-visible:translate-y-0 group-focus-visible:opacity-100">
            {artwork.titleEn}
          </span>
        )}
      </div>
      <h3 className="text-lg leading-tight text-ink transition-colors group-hover:text-cinnabar">
        {artwork.titleEn}
      </h3>
